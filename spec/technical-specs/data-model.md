...
Зафиксировать технические модели данных текущей online Taplink-версии Doctor Amal.

Модели нужны для отображения публичной страницы. Они не описывают заявки, статьи, платежи или аккаунты покупателей.

## Общие правила

- Основной контент текущей реализации хранится в Supabase.
- Rows из Supabase преобразуются в `TaplinkPageData`.
- Подтвержденные локальные данные используются как fallback публичной страницы.
- Активный `doctor_admin` редактирует разрешенные поля через `/admin`.
...
`medicalNotice` для текущей версии:
...
- `photo` должен быть предоставлен владельцем проекта, заменен через подтвержденную админку или заменен только согласованным временным вариантом.
- Если фото загружается через админку, файл должен пройти проверку типа и размера до обновления `doctor_profile`.
...
- Курс не имеет статуса продаж в текущей версии.
- Курс не имеет отдельной страницы.
- Курс не имеет отдельной Telegram-ссылки.
- Удаленный через админку курс не попадает в `Course[]`.
- Безопасное удаление курса хранится технически, например через `deleted_at`, а не через физическое удаление row.
...
- В текущей версии используется одна общая Telegram-ссылка менеджера для всех курсов.
...
- Автоматическая передача названия курса, prefilled message или Telegram bot не входят в текущую версию.

## Модели, которых нет в текущей версии

В текущей Taplink-версии нет моделей:
...
Rows из Supabase приводятся к этим же моделям:

- `doctor_profile` -> `DoctorProfile`;
- `page_settings.medical_notice` -> `TaplinkPageData.medicalNotice`;
- `external_links` -> `ExternalLink[]`;
- `courses` -> `Course[]`;
- `purchase_settings` -> `PurchaseSettings`.

Supabase-таблицы описаны в `spec/technical-specs/supabase-content-source.md`.

Правила:

- UI не должен получать сырые rows из Supabase;
- mapper должен сохранять текущую модель `TaplinkPageData`;
- неподтвержденные, неактивные или неопубликованные rows не должны отображаться как реальные данные;
- rows курсов с техническим признаком удаления не должны отображаться в публичной модели;
- Supabase не добавляет модели заявок, пользователей, заказов, платежей или медицинских данных на первом этапе.
