...
| 68 | Мастер-план до 10/10 | Завершено | `Work plans/Завершенные/068-master-plan-do-10-iz-10.md` | задана шкала качества и последовательность этапов; после планов `090` и `091` текущая версия достигла `10/10` |
...
| 87 | Разбор замечаний ментора по specs | Завершено | `Work plans/Завершенные/087-razbor-zamechaniy-mentora-po-specs.md` | закрыты gaps по password recovery, Supabase unavailable, manual medical review, публикации/кешу; отдельным планом `089` реализован защищенный revalidation `/` после admin-сохранения |
...
| 90 | Финальная приемка текущей версии до 10 из 10 | Завершено | `Work plans/Завершенные/090-finalnaya-priemka-10-iz-10.md` | текущая версия оценена на `9.5/10`; quality, audit, remote migrations/RLS, production `/`, `/admin`, 404 и anon `/admin/revalidate` проверены; остаток до `10/10` — ручная production-проверка входа доктора и сохранения |
| 91 | Подтвердить финальные 10 из 10 | Завершено | `Work plans/Завершенные/091-podtverdit-finalnye-10-iz-10.md` | владелец проекта подтвердил production-вход в `/admin`, безопасное сохранение разрешенного контента и обновление публичной `/`; текущая версия достигла `10/10` |
| 92 | Уточнить процесс работы агента | Завершено | `Work plans/Завершенные/092-utochnit-agents-process.md` | в `AGENTS.md` явно зафиксирован порядок specs -> активный план -> разработка -> проверка -> завершенный план -> Roadmap/changelog -> git; добавлено future-only правило именования новых файлов и папок |
| 93 | Дизайн и эргономика публичной страницы | Завершено | `Work plans/Завершенные/093-dizayn-ergonomika-publichnoy-stranicy.md` | публичная Taplink-страница визуально уточнена по wellness/nutrition/link-in-bio референсам: hero, кнопки, состояния, course accordion и price badge доработаны без изменения сценариев |
| 94 | Specs загрузки фото и удаления курсов | Завершено | `Work plans/Завершенные/094-specs-foto-i-udalenie-kursov.md` | подтверждены specs для загрузки/замены фото через Supabase Storage и безопасного удаления курса через soft delete; физическое удаление row и восстановление через UI оставлены вне этапа |
| 95 | Реализация фото и удаления курсов | Завершено | `Work plans/Завершенные/095-realizaciya-foto-i-udaleniya-kursov.md` | локально добавлены migration, Storage bucket/policies, загрузка фото через `/admin`, обновление `photo_src/photo_alt` и soft delete курса через `deleted_at`; remote применение вынесено в план `096` |
| 96 | Remote migration фото и soft delete | Завершено | `Work plans/Завершенные/096-remote-migration-foto-i-soft-delete.md` | remote Supabase получил migration `20260624000000`: `courses.deleted_at`, bucket `doctor-media`, Storage policies и public course policy; владелец локально проверил `/admin` после migration |
...
Защищенная админка `/admin` использует Supabase Auth. Активный `doctor_admin` может редактировать разрешенные профильные тексты, внешние ссылки, Telegram-ссылку менеджера, существующие курсы, создавать новые курсы, загрузить или заменить фото доктора и безопасно удалить курс из основного списка через soft delete. После успешного сохранения админка вызывает защищенный `POST /admin/revalidate`, который проверяет `doctor_admin` и выполняет server-side `revalidatePath("/")`, чтобы публичная страница `/` обновлялась сразу или почти сразу. Обычный `revalidate: 30` остается fallback, если мгновенный сброс кеша временно не сработал. Физическое удаление курсов из Supabase через UI, восстановление удаленных курсов через UI, управление Auth users, новые роли, заявки, auth покупателей и онлайн-оплата не входят в текущую версию. Remote Supabase migration `20260624000000_add_doctor_media_and_course_soft_delete.sql` применена к project `dagykilvpiacfbwpcluv`; локальная проверка `/admin` после migration подтверждена владельцем проекта.
...
Разбор замечаний ментора по specs завершен 2026-06-22. Закрыты gaps по актуальности `global-spec.md`, восстановлению пароля администратора, поведению при недоступном Supabase/Auth, ручной медицинской проверке admin-контента и правилу публикации/кеша. После дополнительного подтверждения владельца проекта мгновенное или почти мгновенное обновление публичной страницы после admin-сохранения реализовано отдельным планом `089`.

Финальная приемка текущей версии выполнена 2026-06-22 в плане `090`. После нее владелец проекта выполнил ручную production-проверку и подтвердил вход реального доктора в `/admin`, безопасное тестовое сохранение разрешенного контента и обновление публичной `/` после сохранения. Результат зафиксирован в плане `091`. Текущая версия Doctor Amal достигла `10/10` по мастер-плану `068`.

После замечания ментора по BASE10 в плане `092` уточнен `AGENTS.md`: порядок работы нового AI-агента теперь явно описан от specs и активного плана до завершенного плана, Roadmap/changelog и git. Также добавлено правило именования новых технических файлов и папок на будущее без переименования существующей исторической структуры.

В плане `093` выполнен и принят второй референсный дизайн-проход публичной страницы. Текущая страница сохраняет одну Taplink-структуру и specs, но визуально стала ближе к premium wellness/nutrition направлению: светлая hero-поверхность, глубокие зеленые внутренние кнопки, спокойные внешние ссылки с соц-акцентами, улучшенные состояния и более аккуратный course accordion.

В плане `097` подготовлен новый тестовый вариант по предоставленному референсу: публичная страница оформлена как светлая мобильная карточка на приглушенном зеленом фоне, а текущее фото доктора увеличено почти до полной ширины. Функции, данные, тексты и маршруты не менялись; вариант ожидает визуальной оценки владельца.

В плане `098` к тестовому варианту добавлены узкие декоративные полосы с фруктами, овощами и зеленью по референсу. Для них создан отдельный оригинальный asset без текста, людей, таблеток, баночек и медицинских предметов; изображения не размещаются под текстом и не меняют сценарии страницы.

В плане `099` для блока `Курсы` создана отдельная продуктовая полоса с другим набором продуктов, включая целый и разрезанный авокадо. Первая полоса осталась у блока `О докторе`; функциональность страницы не менялась.

В плане `094` подготовлены specs следующего admin-этапа: доктор сможет загрузить или заменить фото через `/admin`, а курс можно будет удалить из публичной страницы и основного списка админки через soft delete. В плане `095` локально реализованы migration, Storage policies, загрузка фото, обновление `photo_src/photo_alt`, фильтрация `deleted_at` и кнопка безопасного удаления курса. В плане `096` remote migration применена к Supabase project `dagykilvpiacfbwpcluv`, read-only проверка подтвердила `courses.deleted_at`, bucket `doctor-media`, Storage policies и public course policy, а владелец локально проверил `/admin` после migration.
