...
Для визуальной проверки временно добавлены placeholder-данные блока `О докторе` и двух курсов. Блоки `О докторе` и `Курсы` раскрываются inline под соответствующей кнопкой и закрываются повторным нажатием.

Следующий слой работы: выполнить визуальную проверку страницы в браузере, затем commit после подтверждения пользователя.
