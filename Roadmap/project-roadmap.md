...
UI входа в админку реализован: `/admin` открывает форму email/password, проверяет активную роль `doctor_admin` через `admin_users`, показывает защищенную оболочку и поддерживает logout. Владелец проекта вручную проверил вход доктора. Формы редактирования контента еще не реализованы.
